...
public class SearchCalActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		calendarView.setOnDateChangeListener(new CalendarView.OnDateChangeListener() {
			...
			public void onSelectedDayChange(CalendarView view, int year, int month, int dayOfMonth) {
				...
				Toast.makeText(SearchCalActivity.this, ""+year+"/"+(month+1)+"/" +dayOfMonth, Toast.LENGTH_SHORT).show();

				SQLiteDatabase db = helper.getReadableDatabase();
				String search = searchDate.toString();
				/*like 을 사용하여 입력한 문자를 포함한 모든 이름 검색*/
				cursor = db.rawQuery("select * from " + CalDBHelper.TABLE_NAME + " where "
						+ CalDBHelper.COL_DATE + " like '%" + search +"%'", null);

			/*검색 결과를 하나만 표시하므로 ContactDto 사용
			여러 개의 검색 결과를 표시하여야 할 경우 결과 개수 만큼 dto 생성 후 ArrayList 등에 저장 필요*/
				CalendarDto item = new CalendarDto();
...
//					+ COL_NAME + " TEXT, " + COL_IMAGE + " INTEGER, " + COL_SUB + " TEXT, " + COL_LINK + " TEXT, " + COL_EX + " TEXT, " + COL_CAT + " TEXT);";
				...
				lvSearchToDo.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
					...
					public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
						...
						new AlertDialog.Builder(SearchCalActivity.this).setTitle("삭제 확인")
